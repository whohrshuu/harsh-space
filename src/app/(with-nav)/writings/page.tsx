import { getAllWritings, WritingMeta } from "@/lib/writings";
...
function groupWritingsByMonth(writings: WritingMeta[]) {
  ...
  writings.forEach((writing) => {
    if (writing.date) {
      ...
      const dateStr = String(writing.date).substring(0, 7); // YYYY-MM
      if (!grouped[dateStr]) {
        grouped[dateStr] = [];
      }
        ...
        <main className="mx-auto max-w-3xl px-4 py-16 space-y-8 ">
            <header className="space-y-2">
                ...
                <p className="text-muted">
                    Collection of my articles and essays on various topics.
                </p>
            </header>
                ...
                {monthKeys.map((monthKey) => (
                    <li key={monthKey}>
                        <div className="relative">
                            <div className="absolute top-0 bottom-0 right-full hidden md:flex items-start gap-3">
